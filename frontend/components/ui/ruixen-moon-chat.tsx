...
function parseRoadmapSteps(markdownText: string) {
  ...
  for (const line of lines) {
    ...
    if (/^(?:###\s+)?\d+\.\s+/.test(trimmed)) {
      ...
      // Remove raw markdown symbols for clean display
          ...
          >
            ...
            {!msg.isUser && (
              <div className="w-full max-w-[90%]">
                              ...
                              {q.options.map((opt: string) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={cn(
                                      "text-left p-3 rounded-xl text-xs font-semibold transition-all border outline-none",
                ...
                {msg.type === "text" && (
                  <div
                    className="prose prose-invert max-w-none text-sm leading-relaxed bg-[#1e293b]/70 border border-white/5 px-4 py-3 rounded-2xl rounded-bl-none border-l-4 border-l-[#FFEF4D] shadow-md"
                    dangerouslySetInnerHTML={{ __html: marked.parse(msg.text) }}
                  />
                )}
              </div>
